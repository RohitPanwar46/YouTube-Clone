  ...
  async function handleLogout(accessToken) {
    try {
      const bRes = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URI}/api/v1/users/logout`,
        {
          method: "POST",
          headers: {
      ...
      {showMobileSearch ? (
        <div className="flex items-center w-full">
          ...
          <form onSubmit={handleSubmit} className="relative flex flex-1 pt-1.5">
            ...
            <button
              type="submit"
              className="absolute right-2 top-1 h-full px-4 bg-transparent rounded-r-full hover:bg-[#303030] transition-all duration-300"
            >
              <svg
